...
function Therapy({ params: { id } }: Props) {
  ...
  async function handleChat(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("https://cddbackend.online/ask", {
        method: "POST",
        headers: {
      ...
      // Format the answer to add line breaks where appropriate
      const formattedAnswer = newAnswer.split(". ").join(".\n");

      // Update the existing document in Firestore with the formatted answer
      await updateDoc(
        doc(
          db,
          "users",
  ...
  const chatEndRef = useRef<HTMLDivElement | null>(null); // Reference to the chat container's bottom
  ...
  }, [chatHistory]); // Triggered whenever chatHistory changes
    ...
    <>
      <div className="flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-2">
          {chatHistory.length === 0 ? (
            <>
              ...
              <p className="text-center font-bold text-white">
                Hi👋 I identify cognitive distortions and engage conversations
                to help relieve stress
              </p>
              <br />
              <p className="text-center text-white">Please say: Hi</p>
            </>
          ) : (
              ...
              <div key={index}>
                ...
                {item.answer !== null && (
                  <div className="flex justify-start space-x-3 mt-4">
                    ...
                    <div
                      className="bg-gray-300 text-black p-3 rounded-lg max-w-xs shadow border whitespace-pre-wrap"
          ...
          <div ref={chatEndRef} /> {/* Empty div to reference for scrolling */}
        </div>
